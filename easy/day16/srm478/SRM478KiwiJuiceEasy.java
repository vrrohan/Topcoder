...
public class SRM478KiwiJuiceEasy {

	public static int[] thePouring(int[] capacities, int[] bottles, int[] fromId, int[] toId) {
		for (int i = 0; i < fromId.length; i++) {
			int bottleFrom = fromId[i];
			int bootleTo = toId[i];
			if (bottles[bottleFrom] + bottles[bootleTo] < capacities[bootleTo]) {
				// pour full kiwi juice
				bottles[bootleTo] += bottles[bottleFrom];
				bottles[bottleFrom] = 0;
			} else {
				bottles[bottleFrom] -= capacities[bootleTo] - bottles[bootleTo];
				bottles[bootleTo] += capacities[bootleTo] - bottles[bootleTo];
			}
		}
		return bottles;
	}
	...
	public static void main(String[] args) {
		...
		int[] finalBottlesVolume = thePouring(new int[] { 14, 35, 86, 58, 25, 62 }, new int[] { 6, 34, 27, 38, 9, 60 },
				new int[] { 1, 2, 4, 5, 3, 3, 1, 0 }, new int[] { 0, 1, 2, 4, 2, 5, 3, 1 });
		for (int b : finalBottlesVolume) {
			System.out.print(b + " ");
		}
	}
