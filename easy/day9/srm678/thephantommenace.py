...
"""
def find(doors, droids) :
    safetyLevel = 0
    for i in range(len(doors)) :
        droidLevel = abs(doors[i]-droids[0])
        for j in range(len(droids)) :
            droidLevel = min(abs(doors[i]-droids[j]), droidLevel)
        safetyLevel = max(safetyLevel, droidLevel)
    return safetyLevel

print(find([2,3,5,7,11], [1,8,13]))
